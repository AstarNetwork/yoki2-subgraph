import {
  ...
  export function handleTransferSingle(event: TransferSingleEvent): void {
    ...
    if (fromAddress == ZERO_ADDRESS) {
      ...
      updateSeason8Condition(toAddress, timestamp);
      updateSeason9Condition(toAddress, timestamp);
      updateSeason10Condition(toAddress, timestamp);
    }
    // Handle burning (to = 0x0)
      ...
      updateSeason8Condition(fromAddress, timestamp);
      updateSeason9Condition(fromAddress, timestamp);
      updateSeason10Condition(fromAddress, timestamp);
    }
    // Handle transfers
    else {
      updateTokenBalance(fromAddress, event.params.id, event.params.value.neg(), timestamp);
